...
export class WsJwtAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        try {
            ...
            if (!user) {
                this.logger.warn(`WebSocket connection rejected: User not found for token payload ${payload.sub}`);
                client.disconnect();
                return false;
            }
            ...
            this.logger.log(`WebSocket authentication successful for user ${user.email} (${user.id})`);
            return true;
