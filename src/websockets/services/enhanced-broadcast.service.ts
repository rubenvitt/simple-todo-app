...
export class EnhancedBroadcastService {
    ...
    constructor(
        private readonly permissionService: EnhancedPermissionService
    ) { }
    ...
    async broadcastWithPermissionFilter(
        ...
        try {
            ...
            const authorizedUsers = await this.permissionService.getUsersWithPermission(listId, filter);
            const authorizedUserIds = new Set(authorizedUsers.map(user => user.userId));

            // Get all connected authenticated sockets
            const authenticatedSockets = Array.from(server.sockets.sockets.values())
    ...
    async broadcastWithRoleSpecificPayload(
        ...
        try {
            ...
            const allAuthorizedUsers = await this.permissionService.getUsersWithPermission(listId);
    ...
    async broadcastToMinimumPermission(
        ...
        await this.broadcastWithPermissionFilter(server, listId, event, data, filter);
    }
    ...
    async broadcastPermissionAwareNotification(
        ...
        try {
            ...
            await this.broadcastWithRoleSpecificPayload(server, listId, event, payload);
    ...
    async broadcastPermissionChange(
        ...
        try {
            ...
            const targetUserSockets = Array.from(server.sockets.sockets.values())
                .filter((socket: AuthenticatedSocket) =>
