...
export class EnhancedPermissionService {
     ...
     */
    async getUserPermission(userId: string, listId: string): Promise<PermissionCheckResult> {
        try {
            ...
            return {
                ...
                isOwner: false,
                canRead: true,
                canEdit: permissionLevel === PermissionLevel.EDITOR || permissionLevel === PermissionLevel.OWNER,
                canDelete: permissionLevel === PermissionLevel.EDITOR || permissionLevel === PermissionLevel.OWNER,
                canManageShares: permissionLevel === PermissionLevel.OWNER
            };

        } catch (error) {
            this.logger.error(`Error checking user permission for user ${userId} on list ${listId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            return {
                hasAccess: false,
                isOwner: false,
                canRead: false,
     ...
     */
    async getUsersWithPermission(listId: string, filter?: BroadcastPermissionFilter): Promise<UserPermission[]> {
        try {
            ...
            // Add owner
            if (!filter || this.shouldIncludeUser(PermissionLevel.OWNER, true, filter)) {
                permissions.push({
            ...
            this.logger.error(`Error getting users with permission for list ${listId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            return [];
        }
    }
     ...
     */
    hasMinimumPermission(userLevel: PermissionLevel, requiredLevel: PermissionLevel): boolean {
        return this.getPermissionHierarchy(userLevel) >= this.getPermissionHierarchy(requiredLevel);
    }
    ...
    private shouldIncludeUser(
        ...
        if (!this.hasMinimumPermission(permissionLevel, filter.requiredPermission)) {
            return false;
        }
        ...
        if (filter.operationType) {
            switch (filter.operationType) {
                ...
                case 'WRITE':
                case 'DELETE':
                    return permissionLevel === PermissionLevel.EDITOR || permissionLevel === PermissionLevel.OWNER;
                case 'MANAGE':
