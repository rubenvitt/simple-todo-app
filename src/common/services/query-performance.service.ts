...
export class QueryPerformanceService {
  ...
  private recordMetrics(metrics: QueryPerformanceMetrics): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      this.logger.debug(
        `Query executed in ${metrics.duration}ms: ${metrics.query}${
          metrics.recordCount !== undefined
            ? ` (${metrics.recordCount} records)`
            : ''
        }`,
      );
    }
  ...
  getPerformanceStats(): {
    ...
    this.performanceMetrics.forEach((metric) => {
      ...
      if (metric.operation) {
        queriesByOperation[metric.operation] =
          (queriesByOperation[metric.operation] || 0) + 1;
      }
    });
