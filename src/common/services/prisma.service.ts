...
export class PrismaService
    ...
    private async connectWithRetry(): Promise<void> {
        while (this.connectionAttempts < this.maxConnectionAttempts && !this.isConnected) {
            try {
                this.logger.log(`Attempting to connect to database (attempt ${this.connectionAttempts + 1}/${this.maxConnectionAttempts})`);
                ...
                this.logger.error(`Failed to connect to database (attempt ${this.connectionAttempts}): ${errorMessage}`);

                if (this.connectionAttempts >= this.maxConnectionAttempts) {
                    this.logger.error('Max connection attempts reached. Could not connect to database');
                    throw new Error(`Could not connect to database after ${this.maxConnectionAttempts} attempts`);
                }

                // Wait before retrying (exponential backoff)
                const waitTime = Math.pow(2, this.connectionAttempts) * 1000;
                this.logger.log(`Waiting ${waitTime}ms before next connection attempt...`);
     ...
     */
    async healthCheck(): Promise<{ status: string; timestamp: string; query_time_ms?: number }> {
        try {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (this.isConnectionError(err) && attempt < maxRetries) {
                    this.logger.log(`Retrying query (attempt ${attempt + 1}/${maxRetries})`);
