...
describe('QueryPerformanceService', () => {
  ...
  describe('trackQuery', () => {
    ...
    it('should handle query failures', async () => {
      ...
      await expect(
        service.trackQuery(mockFailingQuery, queryInfo),
  ...
  describe('getPerformanceStats', () => {
    ...
    it('should calculate average duration correctly', async () => {
      ...
      for (const { duration, query } of queries) {
        ...
        await service.trackQuery(mockQuery, {
          query,
          model: 'test',
          operation: 'findMany',
        });
  ...
  describe('getOptimizationSuggestions', () => {
    it('should suggest index optimization for high slow query rate', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await service.trackQuery(mockSlowQuery, {
          query: `slow-query-${i}`,
          model: 'test',
        });
      }
    ...
    it('should suggest caching for high volume models', async () => {
      ...
      expect(
        suggestions.some((s) =>
          s.includes('Model "user" has high query volume'),
        ),
      ).toBe(true);
    });
  });
  ...
  describe('memory management', () => {
    it('should limit metrics history to prevent memory bloat', async () => {
      ...
      for (let i = 0; i < 1200; i++) {
        // More than the max of 1000
        const mockQuery = jest.fn().mockResolvedValue([]);
        promises.push(
          service.trackQuery(mockQuery, {
            query: `query-${i}`,
            model: 'test',
            operation: 'findMany',
          }),
