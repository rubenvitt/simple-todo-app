...
export class PerformanceController {
    ...
    private getRecommendedActions(stats: any): string[] {
        ...
        if (stats.averageDuration > 1000) {
            actions.push('Average query time is very high - investigate database performance');
        }

        if (stats.totalQueries > 1000) {
            actions.push('High query volume detected - consider implementing caching strategies');
        }
        ...
        topModels.forEach(([model, count]) => {
            if ((count as number) > stats.totalQueries * 0.3) {
                actions.push(`High query volume for ${model} model - consider optimization`);
            }
        });

        if (actions.length === 0) {
            actions.push('Database performance looks healthy - no immediate actions needed');
        }
