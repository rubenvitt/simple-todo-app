...
export class ListPermissionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const requiredPermission = this.reflector.getAllAndOverride<PermissionLevel>(
        ...
        const userPermission = await this.listSharesService.getUserPermissionLevel(user.id, listId);
    ...
    private hasPermission(userPermission: PermissionLevel, requiredPermission: PermissionLevel): boolean {
