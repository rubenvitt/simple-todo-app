...
describe('ListPermissionGuard', () => {
  ...
  describe('canActivate', () => {
    ...
    it('should return true when user has sufficient permission (OWNER >= EDITOR)', async () => {
      ...
      listSharesService.getUserPermissionLevel.mockResolvedValue(
      ...
      // Assert
      expect(result).toBe(true);
      expect(listSharesService.getUserPermissionLevel).toHaveBeenCalledWith(
        'user-1',
        'list-1',
      );
  ...
  describe('hasPermission', () => {
    it('should correctly implement permission hierarchy', () => {
      ...
      expect(
        guardAny.hasPermission(PermissionLevel.OWNER, PermissionLevel.VIEWER),
      ).toBe(true);
      expect(
        guardAny.hasPermission(PermissionLevel.OWNER, PermissionLevel.EDITOR),
      ).toBe(true);
      expect(
        guardAny.hasPermission(PermissionLevel.OWNER, PermissionLevel.OWNER),
      ).toBe(true);

      // Test EDITOR permissions
      expect(
        guardAny.hasPermission(PermissionLevel.EDITOR, PermissionLevel.VIEWER),
      ).toBe(true);
      expect(
        guardAny.hasPermission(PermissionLevel.EDITOR, PermissionLevel.EDITOR),
      ).toBe(true);
      expect(
        guardAny.hasPermission(PermissionLevel.EDITOR, PermissionLevel.OWNER),
      ).toBe(false);

      // Test VIEWER permissions
      expect(
        guardAny.hasPermission(PermissionLevel.VIEWER, PermissionLevel.VIEWER),
      ).toBe(true);
      expect(
        guardAny.hasPermission(PermissionLevel.VIEWER, PermissionLevel.EDITOR),
      ).toBe(false);
      expect(
        guardAny.hasPermission(PermissionLevel.VIEWER, PermissionLevel.OWNER),
      ).toBe(false);
    });
  });
});
