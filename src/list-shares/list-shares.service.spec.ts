...
describe('ListSharesService', () => {
    ...
    describe('shareList', () => {
        ...
        it('should successfully share a list with another user', async () => {
            ...
            // Act
            const result = await service.shareList('user-1', 'list-1', createListShareDto);
        ...
        it('should throw NotFoundException when list is not found or user does not own it', async () => {
            ...
            // Act & Assert
            await expect(service.shareList('user-1', 'list-1', createListShareDto))
                .rejects.toThrow(new NotFoundException('List not found or you do not have permission to share it'));
        });
        ...
        it('should throw ConflictException when trying to share with self', async () => {
            // Arrange
            prismaService.list.findFirst.mockResolvedValue(mockList);
            prismaService.user.findUnique.mockResolvedValue({ ...mockTargetUser, id: 'user-1' });

            // Act & Assert
            await expect(service.shareList('user-1', 'list-1', createListShareDto))
                .rejects.toThrow(new ConflictException('You cannot share a list with yourself'));
        });
        ...
        it('should throw ConflictException when list is already shared with user', async () => {
            ...
            // Act & Assert
            await expect(service.shareList('user-1', 'list-1', createListShareDto))
                .rejects.toThrow(new ConflictException('List is already shared with this user'));
        });
    });
    ...
    describe('getListShares', () => {
        ...
        it('should throw ForbiddenException when user does not have access', async () => {
            ...
            // Act & Assert
            await expect(service.getListShares('user-1', 'list-1'))
                .rejects.toThrow(new ForbiddenException('You do not have access to this list'));
        });
    });
    ...
    describe('updateListShare', () => {
        ...
        it('should successfully update list share permission', async () => {
            ...
            prismaService.list.findFirst.mockResolvedValue(mockList);
            prismaService.listShare.findUnique.mockResolvedValue(mockListShare);
            const updatedShare = { ...mockListShare, permissionLevel: PermissionLevel.VIEWER };
            prismaService.listShare.update.mockResolvedValue(updatedShare);

            // Act
            const result = await service.updateListShare('user-1', 'list-1', 'user-2', updateListShareDto);
        ...
        it('should throw NotFoundException when user does not own the list', async () => {
            ...
            // Act & Assert
            await expect(service.updateListShare('user-1', 'list-1', 'user-2', updateListShareDto))
                .rejects.toThrow(new NotFoundException('List not found or you do not have permission to modify shares'));
        });
    ...
    describe('removeListShare', () => {
        it('should successfully remove list share', async () => {
            ...
            // Act
            const result = await service.removeListShare('user-1', 'list-1', 'user-2');
        ...
        it('should throw NotFoundException when user does not own the list', async () => {
            ...
            // Act & Assert
            await expect(service.removeListShare('user-1', 'list-1', 'user-2'))
                .rejects.toThrow(new NotFoundException('List not found or you do not have permission to modify shares'));
        });
