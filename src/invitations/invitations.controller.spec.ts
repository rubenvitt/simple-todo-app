...
describe('InvitationsController', () => {
    ...
    describe('createInvitation', () => {
        ...
        it('should successfully create an invitation', async () => {
            ...
            const result = await controller.createInvitation('list-1', createInvitationDto, req);
    ...
    describe('getPendingInvitations', () => {
        ...
        it('should return pending invitations for user', async () => {
            ...
            invitationsService.getPendingInvitationsForUser.mockResolvedValue(invitations);

            const result = await controller.getPendingInvitations(req, {});

            expect(result).toEqual(invitations);
            expect(invitationsService.getPendingInvitationsForUser).toHaveBeenCalledWith(
                'user@example.com',
            );
        });
        ...
        it('should use paginated version when query parameters are provided', async () => {
            ...
            invitationsService.getPaginatedInvitationsForUser.mockResolvedValue(paginatedResult);

            const result = await controller.getPendingInvitations(req, { page: 1, limit: 10 });

            expect(result).toEqual(paginatedResult);
            expect(invitationsService.getPaginatedInvitationsForUser).toHaveBeenCalledWith(
                'user@example.com',
                { page: 1, limit: 10 },
            );
        });
    ...
    describe('getInvitationsByList', () => {
        ...
        it('should return invitations for a list', async () => {
            ...
            expect(result).toEqual(invitations);
            expect(invitationsService.getInvitationsByList).toHaveBeenCalledWith('list-1', 'user-1');
        });
        ...
        it('should throw NotFoundException when list not found or user is not owner', async () => {
            ...
            await expect(controller.getInvitationsByList('list-1', req)).rejects.toThrow(
                NotFoundException,
            );
        });
    });
    ...
    describe('acceptInvitation', () => {
        ...
        it('should throw NotFoundException when invitation not found', async () => {
            ...
            await expect(controller.acceptInvitation('invalid-token', req)).rejects.toThrow(
                NotFoundException,
            );
        });
    });
    ...
    describe('declineInvitation', () => {
        ...
        it('should successfully decline invitation', async () => {
            const declinedInvitation = { ...mockInvitation, status: InvitationStatus.DECLINED };
            invitationsService.declineInvitation.mockResolvedValue(declinedInvitation);
        ...
        it('should throw NotFoundException when invitation not found', async () => {
            ...
            await expect(controller.declineInvitation('invalid-token', req)).rejects.toThrow(
                NotFoundException,
            );
        });
    });
