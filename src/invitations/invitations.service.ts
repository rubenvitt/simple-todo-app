...
export class InvitationsService {
  ...
  async createInvitation(
    ...
    if (existingInvitation) {
      throw new BadRequestException(
        'Invitation already sent to this email for this list',
      );
    }
    ...
    if (inviteeUser) {
      ...
      if (existingShare) {
        throw new BadRequestException(
          'This user already has access to this list',
        );
      }
    }
  ...
  async handleExpiredInvitationsCleanup(): Promise<void> {
    try {
      ...
      if (expiredCount > 0) {
        this.logger.log(
          `Successfully marked ${expiredCount} expired invitations as EXPIRED`,
        );
      } else {
