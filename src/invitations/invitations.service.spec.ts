...
describe('InvitationsService', () => {
    ...
    describe('createInvitation', () => {
        ...
        beforeEach(() => {
            ...
            jest.spyOn(require('crypto'), 'randomBytes').mockReturnValue(Buffer.from('sample-token-bytes'));
        });
        ...
        it('should successfully create an invitation', async () => {
            ...
            const result = await service.createInvitation('list-1', createInvitationDto, 'user-1');
        ...
        it('should throw BadRequestException for self-invitation', async () => {
            prismaService.list.findFirst.mockResolvedValue(mockList);
            prismaService.user.findUnique.mockResolvedValue({ ...mockUser, id: 'user-1' });
    ...
    describe('getPendingInvitationsForUser', () => {
        it('should return pending invitations for user', async () => {
            ...
            const result = await service.getPendingInvitationsForUser('invitee@example.com');
    ...
    describe('acceptInvitation', () => {
        it('should successfully accept invitation', async () => {
            const updatedInvitation = { ...mockInvitation, status: InvitationStatus.ACCEPTED };
            ...
            const result = await service.acceptInvitation('sample-token', 'invitee@example.com');
    ...
    describe('declineInvitation', () => {
        it('should successfully decline invitation', async () => {
            const updatedInvitation = { ...mockInvitation, status: InvitationStatus.DECLINED };

            prismaService.invitation.findFirst.mockResolvedValue(mockInvitation);
            prismaService.invitation.update.mockResolvedValue(updatedInvitation);

            const result = await service.declineInvitation('sample-token', 'invitee@example.com');
    ...
    describe('handleExpiredInvitationsCleanup', () => {
        it('should successfully run scheduled cleanup', async () => {
            const cleanupSpy = jest.spyOn(service, 'cleanupExpiredInvitations').mockResolvedValue(3);
            const loggerSpy = jest.spyOn(service['logger'], 'log').mockImplementation();

            await service.handleExpiredInvitationsCleanup();

            expect(cleanupSpy).toHaveBeenCalled();
            expect(loggerSpy).toHaveBeenCalledWith('Starting scheduled cleanup of expired invitations...');
            expect(loggerSpy).toHaveBeenCalledWith('Successfully marked 3 expired invitations as EXPIRED');
        });

        it('should handle cleanup with no expired invitations', async () => {
            const cleanupSpy = jest.spyOn(service, 'cleanupExpiredInvitations').mockResolvedValue(0);
            const loggerDebugSpy = jest.spyOn(service['logger'], 'debug').mockImplementation();

            await service.handleExpiredInvitationsCleanup();

            expect(cleanupSpy).toHaveBeenCalled();
            expect(loggerDebugSpy).toHaveBeenCalledWith('No expired invitations found during cleanup');
        });

        it('should handle cleanup errors gracefully', async () => {
            const error = new Error('Database connection failed');
            const cleanupSpy = jest.spyOn(service, 'cleanupExpiredInvitations').mockRejectedValue(error);
            const loggerErrorSpy = jest.spyOn(service['logger'], 'error').mockImplementation();

            await service.handleExpiredInvitationsCleanup();

            expect(cleanupSpy).toHaveBeenCalled();
            expect(loggerErrorSpy).toHaveBeenCalledWith('Failed to cleanup expired invitations', error);
        });
    });
