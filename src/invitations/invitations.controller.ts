...
export class InvitationsController {
  ...
  @Get()
  async getPendingInvitations(@Request() req: any, @Query() query: any) {
    // If pagination parameters are provided, use paginated version
    if (query.page || query.limit || query.status || query.search) {
      return this.invitationsService.getPaginatedInvitationsForUser(
        req.user.email,
        query,
      );
    }
  ...
  async cleanupExpiredInvitations(): Promise<{
    ...
    const expiredCount =
      await this.invitationsService.cleanupExpiredInvitations();
    return {
